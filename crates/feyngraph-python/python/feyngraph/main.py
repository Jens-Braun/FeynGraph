...
from logging import basicConfig, critical, getLogger, info, warning
...
try:
    ...
    print(
        "Unable to import all dependencies, please install FeynGraph with the optional 'cli' feature, e.g. 'pip install feyngraph[cli]'."
    )
    sys.exit()
...
def main():
    ...
    cli_parser = argparse.ArgumentParser(
        prog="FeynGraph",
        description="A modern Feynman diagram generation toolkit",
        formatter_class=RichHelpFormatter,
    )
    cli_parser.add_argument(
        "config", help="Configuration file for the Diagram generation in TOML format"
    )
    cli_parser.add_argument(
        "-t",
        "--template",
        help="Jinja2 template to render the output with",
        default=None,
    )
    cli_parser.add_argument(
        "-o",
    ...
    cli_parser.add_argument(
        "-j", "--jobs", help="Number of worker threads to use", type=int, default=None
    )
    cli_parser.add_argument(
        "--loglevel", choices=["DEBUG", "INFO", "WARN", "ERROR"], default="INFO"
    )
    ...
    if "template" not in conf:
        info(
            f"Config file {cli_args.config} does not contain property 'template', defaulting to 'json.jinja'."
        )
        conf["template"] = "json.jinja"
    if "process" not in conf:
        critical(
            f"Config file {cli_args.config} does not contain a '[process]' section."
        )
        sys.exit()
    if "in" not in conf["process"] or "out" not in conf["process"]:
        critical(
            f"Config file {cli_args.config} does not contain the required 'in' and 'out' properties in the '[process]' section."
        )
        sys.exit()
    if "loops" not in conf["process"]:
        info(
            f"Config file {cli_args.config} does not contain property 'loops' in the '[process]' section, defaulting to 'loops = 0'."
        )
        conf["process"]["loops"] = 0
    if "model" not in conf["process"]:
        info(
            f"Config file {cli_args.config} does not contain property 'model' in the '[process]' section, defaulting to the Standard Model in Feynman gauge."
        )
        conf["process"]["model"] = None
    if "momenta" not in conf["process"]:
        info(
            f"Config file {cli_args.config} does not contain property 'momenta' in the '[process]' section, defaulting to 'p<i>' for external momenta and 'l<i>' for loop momenta."
        )
        ...
        if len(conf["process"]["momenta"]) != n_legs + n_loops:
            critical(
                f"The given process with {n_legs} legs and {n_loops} loops has {n_legs + n_loops} associated momenta, but {len(conf['process']['momenta'])} labels were specified. Please specify exactly {n_legs + n_loops} labels."
            )
            sys.exit()
    if "drawing" in conf:
        if "format" not in conf["drawing"]:
            conf["drawing"]["format"] = "tikz"
        elif conf["drawing"]["format"] != "tikz" and conf["drawing"]["format"] != "svg":
            critical(
                f"Unknown drawing format '{conf['drawing']['format']}', the supported formats are 'tikz' and 'svg'."
            )
            sys.exit()
        if "outdir" not in conf["drawing"]:
            conf["drawing"]["outdir"] = "feyngraph_drawings"
        if "filename" not in conf["drawing"]:
    ...
    if conf["process"]["model"] is not None:
        if os.path.isfile(conf["process"]["model"]):
            ...
            critical(
                f"Unable to import model {conf['process']['model']} because it is not a valid QGRAF or UFO model."
            )
            sys.exit()
        info(
            f"Successfully imported model {conf['process']['model']} with {len(model.particles())} particles and {len(model.vertices())} vertices."
        )
    else:
        info("Loading Standard Model in Feynman Gauge...")
        model = fg.Model()
        info(
            f"Successfully imported Standard Model with {len(model.particles())} particles and {len(model.vertices())} vertices."
        )
        ...
        for key, _ in conf["filter"].items():
            match key:
                ...
                case "custom_function":
                    if "custom_code" not in conf["filter"]:
                        critical(
                            f"Custom filter function '{conf['filter']['custom_function']}' requested in config, but no code to define it was given."
                        )
                        sys.exit()
                    exec(conf["filter"]["custom_code"], globals())
                    selector.add_custom_function(
                        globals()[conf["filter"]["custom_function"]]
                    )
                case "custom_code":
                    if "custom_function" not in conf["filter"]:
                        warning(
                            "The config contains custom filter code, but no custom filter function was defined."
                        )
                    continue
                case _:
    ...
    n_legs = len(conf["process"]["in"]) + len(conf["process"]["out"])
    info(
        f"Generating Feynman diagrams with {n_legs} legs and {conf['process']['loops']} loops."
    )
    with console.status("Generating diagrams...", spinner="bouncingBall"):
        diag_gen = fg.DiagramGenerator(
            conf["process"]["in"],
            conf["process"]["out"],
            conf["process"]["loops"],
            model=model,
    ...
    template_path: str = conf["template"]
    if os.path.isfile(template_path):
        with open(template_path, "r") as f:
            template = jinja2.Template(f.read())
    else:
        fg_prefix = pathlib.Path(
            cast(str, cast(ModuleSpec, importlib.util.find_spec("feyngraph")).origin)
