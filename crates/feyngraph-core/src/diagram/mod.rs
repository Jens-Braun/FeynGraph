...
use model::{Model, ModelError, TopologyModel};
use model::{ModelBase, ParticleBase};
...
impl<M: ModelBase> Diagram<M> {
    fn from(workspace: &AssignWorkspace<M>, vertex_symmetry: usize) -> Self {
        ...
        for ((vertices, particle), count) in propagators.iter().counts_by(|prop| (prop.vertices, prop.particle)) {
            if vertices[0] == vertices[1] {
                if workspace.model.particle(particle).is_self_anti() {
        ...
        if workspace.model.supports_sign() {
            d.sign =
                DiagramView::new(workspace.model.as_ref(), &d, workspace.momentum_labels.as_ref()).calculate_sign();
        }
...
#[derive(Debug)]
pub struct DiagramContainer<M: ModelBase> {
    pub(crate) model: Option<Arc<Model<M>>>,
    pub(crate) momentum_labels: Arc<Vec<String>>,
    pub(crate) data: Vec<Diagram<M>>,
}

impl<M: ModelBase> DiagramContainer<M> {
    #[cfg(not(feature = "_internal_api"))]
    pub(crate) fn new(model: Option<Arc<Model<M>>>, momentum_labels: &[String]) -> Self {
    ...
    #[cfg(feature = "_internal_api")]
    pub fn new(model: Option<Arc<Model<M>>>, momentum_labels: &[String]) -> Self {
        Self {
            model: model,
            momentum_labels: Arc::new(momentum_labels.to_owned()),
            data: Vec::new(),
    ...
    pub(crate) fn with_capacity(model: Option<Arc<Model<M>>>, momentum_labels: &[String], capacity: usize) -> Self {
        Self {
            model: model,
            momentum_labels: Arc::new(momentum_labels.to_owned()),
    ...
    fn inner_ref_mut(&mut self) -> &mut Vec<Diagram<M>> {
    ...
    pub fn get(&self, i: usize) -> DiagramView<'_, M> {
    ...
    pub fn views(&self) -> impl Iterator<Item = DiagramView<'_, M>> {
        return self
            .data
            .iter()
    ...
    pub fn view(&self, i: usize) -> DiagramView<'_, M> {
        return DiagramView::new(self.model.as_ref().unwrap(), &self.data[i], &self.momentum_labels);
    }
    ...
    pub fn query(&self, selector: &DiagramSelector<M>) -> Option<usize> {
    ...
    pub fn query_function(&self, f: impl Fn(&DiagramView<M>) -> bool) -> Option<usize> {
        return if let Some((i, _)) = self.data.iter().find_position(|diagram| {
            f(&DiagramView::new(
    ...
    pub fn model(&self) -> Arc<Model<M>> {
        return self.model.as_ref().unwrap().clone();
    }

    pub fn momentum_labels(&self) -> Arc<Vec<String>> {
        return self.momentum_labels.clone();
    }
}

impl<M: ModelBase> From<Vec<DiagramContainer<M>>> for DiagramContainer<M> {
    fn from(containers: Vec<DiagramContainer<M>>) -> Self {
        if containers.is_empty() {
            return DiagramContainer::new(None, &[]);
        }
        let mut result = DiagramContainer::with_capacity(
            containers[0].model.clone(),
...
#[cfg(feature = "_internal_api")]
impl<M: ModelBase> Deref for DiagramContainer<M> {
    type Target = Vec<Diagram<M>>;
...
/// # Examples
/// ```rust
/// # use feyngraph_core::{DiagramGenerator, DiagramSelector};
/// # use model::Model;
///
/// let model = Model::sm();
...
/// ```
pub struct DiagramGenerator<M: ModelBase> {
    model: Arc<Model<M>>,
...
impl<M: ModelBase> DiagramGenerator<M> {
    ...
    pub fn new(
        ...
        n_loops: usize,
        model: Model<M>,
        selector: Option<DiagramSelector<M>>,
    ) -> Result<Self, ModelError> {
        let incoming_particles: Vec<usize> = particles_in
            .iter()
            .map(|p| model.particle_index_by_name(p))
            .collect::<Result<_, _>>()?;
        ...
        let outgoing = outgoing_particles
            .into_iter()
            .map(|p| model.anti_particle_index(p))
            .collect_vec();
        let used_selector;
        if let Some(selector) = selector {
            used_selector = selector;
        } else {
            used_selector = DiagramSelector::<M>::new();
        }
        return Ok(Self {
            model: Arc::new(model),
    ...
    pub fn generate(&self) -> DiagramContainer<M> {
        ...
        let mut containers: Vec<DiagramContainer<M>> = Vec::new();
        topologies
            .inner_ref()
            .into_par_iter()
            .map(|topology| {
        ...
        container.model = Some(self.model.clone());
        return container;
    }
    ...
    pub fn assign_topology(&self, topology: &Topology) -> Result<DiagramContainer<M>, InputError> {
    ...
    pub fn assign_topologies(&self, topologies: &[Topology]) -> Result<DiagramContainer<M>, InputError> {
...
mod tests {
    ...
    pub fn diagram_generator_sign_test() {
        ...
        assert_eq!(diags.view(0).sign(), -diags.view(1).sign());
    }
