...
impl Particle {
    ...
    /// Get the particle's typeset name. Corresponds to the UFO property `texname`.
    pub fn texname(&self) -> &String {
        return &self.texname;
    }
...
impl Model {
    ...
    /// # use feyngraph_core::Model;
    /// let model = Model::from_ufo(&PathBuf::from("../../tests/models/Standard_Model_UFO")).unwrap();
    /// ```
    ...
    /// # use std::path::PathBuf;
    /// # use feyngraph_core::Model;
    /// let model = Model::from_qgraf(&PathBuf::from("../../tests/models/sm.qgraf")).unwrap();
    /// ```
...
/// # Examples
/// ```rust
/// # use feyngraph_core::topology::TopologyModel;
