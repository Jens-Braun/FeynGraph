...
use crate::{
    drawing::{Decoration, backend::Backend, math::Vec2D, style::Theme},
    model::LineStyle,
};

pub(crate) struct CanvasGrid<B: Backend> {
    backend: B,
    cols: usize,
    rows: usize,
}
...
impl<B: Backend> CanvasGrid<B> {
    ...
    pub(crate) fn finish(self) -> B::Output {
        return self.backend.finish();
    }

    pub(crate) fn draw_title(&mut self, col: usize, row: usize, label: &str) {
        let theme = Theme::get_global();
...
pub(crate) struct Canvas<'a, B: Backend> {
    backend: &'a mut B,
    origin: Vec2D,
    labels: Vec<Label>,
    paths: Vec<Path>,
...
impl<'a, B: Backend> Canvas<'a, B> {
    pub(crate) fn push_label(&mut self, text: String, pos: Vec2D, anchor: Anchor, math: bool) {
    ...
    pub(crate) fn push_decoration(&mut self, pos: Vec2D, d: Decoration) {
